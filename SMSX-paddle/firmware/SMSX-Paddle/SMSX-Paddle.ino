...
unsigned char value = 0x00;  //stolen from export paddle code
...
void setup() {
  ...
  if (digitalRead(_nibbleGND)) {
    if (!digitalRead(_fireButton)) {  //hold button while plugging in for export mode
      hostSystem = _export;
      initializeExport();
    } else {
      hostSystem = _SMS;
  ...
  if (hostSystem == _MSX)
    Serial.println("MSX");
  else if (hostSystem == _SMS)
    Serial.println("SMS");
  else if (hostSystem == _export)
...
void initializeSMS(void) {
  ...
  pinMode(_buttonClock, OUTPUT);  //no idea why it's not always an output for SMS use
  pinMode(_nibbleGND, OUTPUT);
  pinMode(_d0d4data, OUTPUT);
  pinMode(_d1d5button, OUTPUT);
  pinMode(_d2d6, OUTPUT);
  pinMode(_d3d7, OUTPUT);
...
///////////////////////////////////////////////////////////////////////////////
// Perform hardware initialization for SMS in export mode
void initializeExport(void) {

  // I think I can get away with just a polling loop, no interrupts
  pinMode(_buttonClock, OUTPUT);  //no idea why it's not always an output for SMS use
  pinMode(_nibbleGND, OUTPUT);    //needed for handshake
  pinMode(_d0d4data, OUTPUT);
  pinMode(_d1d5button, OUTPUT);
  pinMode(_d2d6, OUTPUT);
  pinMode(_d3d7, OUTPUT);
  PCICR |= B00000100;   //PORTD interrupt
  PCMSK2 |= B00010000;  //D4 interrupt (fire button)
}
///////////////////////////////////////////////////////////////////////////////
...
///////////////////////////////////////////////////////////////////////////////
// Run SMS export paddle emulation
void doSmsExport(void) {

  PORTB = ((value >> 4) & 0x0f) | 0x10;  //lower 4 bits of port b and TR to trigger it as needed for handshake, this is the MSB
  while (PIND & 0x40) {                  // updated for PD6
  }

  // New value on falling edge
  value = ADCH;

  PORTC = (value << 0) & 0x0f;  //this is LSB and it's already aligned
  while (!(PIND & 0x40)) {      // updated for PD6
  }
...
ISR(PCINT2_vect) {    // Interrupt for Port D
  if (PIND & 0x10) {  //if fire button (PD4) is high
    PORTD |= 0x08;    //set PD3 high
  } else {
    PORTD &= 0xF7;  //set PD3 low
  }
}
